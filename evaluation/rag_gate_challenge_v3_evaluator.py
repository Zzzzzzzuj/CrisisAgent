...
PROTOCOL_PATH = PROJECT_ROOT / "evaluation" / "reports" / "rag_gate_challenge_v3_protocol.md"
REPORT_PATH = PROJECT_ROOT / "evaluation" / "reports" / "latest_rag_gate_challenge_v3.md"
...
class DisabledFallbackRetriever:
    def retrieve(self, query: str, top_k: int = TOP_K) -> RetrievalResult:
        raise RuntimeError("Fallback is disabled for Challenge v3 Evaluation.")
...
def validate_challenge_cases(cases: list[dict]) -> None:
    ...
    if len(cases) != 40:
        raise ValueError(f"Challenge v3 must contain 40 cases, got {len(cases)}.")
    if len(ids) != len(set(ids)):
        raise ValueError("Challenge v3 contains duplicate ids.")
    ...
    if len(positives) != 20 or len(negatives) != 20:
        raise ValueError("Challenge v3 must contain 20 positive and 20 negative cases.")
...
def summarize_gate_results(case_results: list[dict]) -> dict:
    ...
    hard_negative_cases = [case for case in negative_cases if case["type"] == "hard_negative"]
    weak_current_cases = [case for case in positive_cases if case.get("weak_current_incident")]
    non_current_hard_cases = [case for case in hard_negative_cases if case.get("intent_type")]
    ...
    return {
        **counts,
        ...
        "current_incident_positive_pass_rate": _positive_pass_rate(weak_current_cases),
        "current_incident_positive_count": len(weak_current_cases),
        "non_current_hard_negative_reject_rate": _negative_reject_rate(non_current_hard_cases),
        "non_current_hard_negative_count": len(non_current_hard_cases),
    }
...
def summarize_end_to_end(case_results: list[dict]) -> dict:
    ...
    return {
        ...
        "gate_false_negative_count": sum(
            1 for case in case_results if case["failure_reason"] == "gate_false_negative"
        ),
        "retrieval_miss_count": sum(
            1 for case in case_results if case["failure_reason"] in {"no_hit", "miss"}
        ),
        "retrieval_pollution_count": sum(
            1 for case in case_results if case["failure_reason"] == "wrong_category"
        ),
...
def build_markdown_report(result: dict) -> str:
    ...
    lines = [
        ...
        f"- gate_v3_commit: `{result['gate_v3_commit']}`",
        ...
        f"- hard_negative_reject_count: `{gate['hard_negative_reject_count']} / 12`",
        f"- current_incident_positive_pass_rate: `{gate['current_incident_positive_pass_rate']}`",
        f"- non_current_hard_negative_reject_rate: `{gate['non_current_hard_negative_reject_rate']}`",
        "",
        "## Positive Category TPR",
        "",
    ]
    for category, value in gate["positive_by_category"].items():
        lines.append(f"- `{category}`: `{value}`")
    lines.extend(["", "## Negative Type Reject Rate", ""])
    for case_type, value in gate["negative_by_type"].items():
        lines.append(f"- `{case_type}`: `{value}`")
    lines.extend(
        [
            "",
            "## End-to-End Metrics",
            ...
            f"- Negative no-hit by type: `{end_to_end['negative_no_hit_by_type']}`",
            ...
            f"- Gate False Negative Count: `{end_to_end['gate_false_negative_count']}`",
            f"- Retrieval Miss Count: `{end_to_end['retrieval_miss_count']}`",
            f"- Retrieval Pollution Count: `{end_to_end['retrieval_pollution_count']}`",
            "",
            "## Acceptance Checks",
            "",
        ]
    ...
    lines.extend(
        [
            f"- Overall Gate Challenge v3 Status: `{acceptance['status']}`",
            "",
            "## False Positives",
            "",
        ]
...
def _negative_reject_rate(cases: list[dict]) -> float:
    if not cases:
        return 0.0
    return _safe_rate(_negative_reject_count(cases), len(cases))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Retrieval Need Gate Challenge v3 Evaluation.")
        ...
        json.dumps(
            {
                ...
                "gate_v3_commit": result["gate_v3_commit"],
                "gate": result["gate"],
                "end_to_end": {
                    ...
                    "gate_false_negative_count": result["end_to_end"]["gate_false_negative_count"],
                    "retrieval_miss_count": result["end_to_end"]["retrieval_miss_count"],
                    "retrieval_pollution_count": result["end_to_end"]["retrieval_pollution_count"],
                },
                "acceptance": result["acceptance"],
