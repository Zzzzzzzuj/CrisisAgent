...
def test_knowledge_governance_migration_is_after_pgvector():
    migration = Path("alembic/versions/0005_add_knowledge_governance_fields.py").read_text(encoding="utf-8")

    assert 'revision = "0005_knowledge_governance"' in migration
    assert 'down_revision = "0004_pgvector"' in migration
    assert "status" in migration
    assert "is_enabled" in migration
    assert "source_name" in migration
